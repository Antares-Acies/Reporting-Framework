#This file is responsible for picking up the Cashflows - parquet value from the revolutio
...
logging.warning(f"  ")
logging.warning(f"  ")
logging.warning("Editor start")
logging.warning(f"  ")
logging.warning(f"Start of cashflow editor")
...
logging.warning(f"DISKSTORE_PATH: {DISKSTORE_PATH}")

# Set up the directory for cashflow files
cashflow_dir = Path(f"{DISKSTORE_PATH}Cashflow_Engine_Outputs/Cashflow")
logging.warning(f"cashflow_dir: {cashflow_dir}")


# 1) Find all .parquet files that match the valuation date pattern

all_parquet_files = list(cashflow_dir.glob("*.parquet"))
logging.warning(f"Number of .parquet files found in {cashflow_dir}: {len(all_parquet_files)}")

all_files_for_date = list(cashflow_dir.glob(f"*_{valuation_date_str}_*.parquet"))
...
if not all_files_for_date:
    ...
    final_cashflow_df = pd.DataFrame()  # Return or handle empty DataFrame
else:
    ...
    for file_path in all_files_for_date:
        ...
        # Sample path file:- 'cashflow_output_2024-07-31_EMI Loans_1_run_123456789.parquet'
    ...
    final_cashflow_df = pd.DataFrame()
    ...
    for variant_name, file_list in variant_to_files.items():
        ...
        if dfs_for_variant:
            ...
            final_cashflow_df = pd.concat(
                [final_cashflow_df, variant_df], ignore_index=True
            )
    ...
    if not final_cashflow_df.empty:
        total_mem_usage_mb = final_cashflow_df.memory_usage(deep=True).sum() / (1024**2)
        logging.warning(
...
output_data = final_cashflow_df.astype(str)

logging.warning("Converting to STR")
logging.warning("Editor end of cashflow report.")
logging.warning("  ") 
logging.warning("  ")