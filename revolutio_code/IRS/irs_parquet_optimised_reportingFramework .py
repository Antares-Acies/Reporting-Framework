#IRS REPORT
import logging
print("Running IRS-TGA integration report")
import numpy as np
import numpy as np
import logging
import pandas as pd
...
currency_pair_master = Data21.astype('object')

currency_conversion_master = Data24.astype('object')


quoted_security_data = Data20.astype('object')
quoted_security_data.drop_duplicates(subset = ['security_identifier'],inplace = True)

currency_scenario_config = Data10

currency_conversion_exemption = Data11
...
print(f"Columns to be calculated: {calculated_columns}")
...
print(f"Bucketing applicability  bucketing_flag_global  flag: {bucketing_flag_global}")
...
bucket_definition = Data15.astype('object')

bucket_rule_mapping = Data16.astype('object')

bucketing_type = Data12.astype('object')

rule_based_bucketing = Data14.astype('object')

static_pattern_bucketing = Data17.astype('object')

reporting_pattern_bucketing = Data18.astype('object')

reporting_bucketing_adjustment = Data13.astype('object')
...
if 'Yes' in column_type['bucketing_applicability'].values:
    ...
    print("Bucketing-related data read successfully.")
...
print("Reading limit setup data...")
global limit_setup
limit_setup = Data9.astype('object')
# limit_setup.rename(columns={'limit_value': 'limit'}, inplace=True)

## Importing all system tables
global position_data
position_data = Data4.astype('object')

cashflow_report = Data1.astype('object')

valuation_report = Data27.astype('object')
...
gl_master = Data3.astype('object')

print(f"len of gl master: {len(gl_master)}")
print(f" 2 len of gl master: {len(gl_master)}")
...
for key, df in dataframes.items():
  time.sleep(1)
  print(f"The shape of the dataframe '{key}' is: {df.shape}")
...
def read_dataframes(dataframes):
    ...
    print(f"Unique source sheets to read: {unique_source_sheet}")
    ...
    for sheet in unique_source_sheet:
        try:
            ...
            print(f"Sheet {sheet} read successfully.")
        except Exception as e:
            print(f"EXCEL SHEET {sheet} is MISSING: {e}")
            
    print("exit read_dataframe function")
...
def dynamic_merge(merge_master, dataframes):
    ...
    def do_merge_step(left_table, right_table, left_keys, right_keys):
        ...
        """
        print(f"  ")
        print(f"Merging {left_table} with {right_table}")
        print(f"left_keys: {left_keys}, right_keys: {right_keys}")
        ...
        if left_table in merged_data:
            ...
            if left_table not in dataframes:
                print(f"Left table {left_table} not in `dataframes`. Skipping.")
                return
        ...
        if right_table in merged_data:
            ...
            if right_table not in dataframes:
                print(f"Right table {right_table} not in `dataframes`. Skipping.")
                return
            right_df = dataframes[right_table]
        
        # --- 2. Log shapes ---
        print(f"Before merge: {left_table} shape = {left_df.shape}")
        print(f"Before merge: {right_table} shape = {right_df.shape}")
        ...
        print(f"After merge: {left_table} shape = {merged_df.shape}")
    ...
    print(f"  ")
    print("All merges completed in config table order.")
    time.sleep(10)
    return merged_data
    ...
    for index, row in merge_master.iterrows():
        ...
        try:
          if table_name in merged_data:
              print('Line No 486: Present in if block')
              all_dataframes_dict[table_name] = merged_data[table_name]
          else:
              print('Line No 486: Present in else block')
              if table_name in dataframes:
                  print(f'This Table : {table_name} is not found. The size is {dataframes[table_name].shape}.')
                  all_dataframes_dict[table_name] = dataframes[table_name]
              else:
                  print(f'This Table : {table_name} is not found. Initializing an empty DataFrame.')
...
def conversion(dataframes, currency_conversion_master, currency_conversion_rate, base, reporting_currency):
    ...
    """
    print(f" currency conversion fxn start ")
    for index, row in base.iterrows():
        ...
        print(f" exemption_rows {exemption_rows} ")
        
        if not exemption_rows.empty:
            print(f" currency conversion exemption is not empty ")
            ...
            for _, exemption_row in exemption_rows.iterrows():
                ...
                print(f" full_exemption_condition_column {full_exemption_condition_column} ")
                ...
                print(f"len exemption_mask {len(exemption_mask)} ")
...
def evaluate_condition(df, condition):
    ...
    if condition_datatype is None:
        print("KeyError: 'condition_datatype' not found in condition dictionary")
        return pd.Series([True] * len(df), index=df.index)

    condition_column_name = condition['condition_column_name']
    if condition_column_name not in df.columns:
        print(f"Failed to find the target column: {condition_column_name}")
    ...
    try:
        if condition_datatype == 'Integer':
            ...
            print(f" inside integer conversion for column ")
            df[condition_column_name] = df[condition_column_name].astype(int)
            ...
            print(f"Unknown condition_datatype: {condition_datatype}")
            return pd.Series([True] * len(df), index=df.index)
    except Exception as e:
        print(f"Error converting column to datatype {condition_datatype}: {e}")
        return pd.Series([True] * len(df), index=df.index)
    ...
    print(f" final condition_value post conversion's and all : {condition_value}")
    ...
    if condition['condition_type'] == 'Contains':
        ...
        print(f"Mapping values for 'Contains': {mapping_values}")
        return df_column.isin(mapping_values)
    elif condition['condition_type'] == 'Does not Contain':
        mapping_values = mapping_set[mapping_set['mapping_set'] == condition_value]['mapping_criteria'].tolist()
        print(f"Mapping values for 'Does not Contain': {mapping_values}")
        return ~df_column.isin(mapping_values)
        ...
        print(f"Unknown condition type: {condition['condition_type']}")
        # Return a Series of True with the same index as df
        return pd.Series([True] * len(df), index=df.index)
...
def filter_dataframes_by_currency(dataframes, currency_conversion_master, currency_list):
    ...
    """
    print("Inside filter_dataframes_by_currency")
...
def combine_bucketed_values(dict1, dict2, operation, operation_parameter=None):
    ...
    """
    print(f"inside combine_bucketed_values fxn ")
    ...
    if operation == 'CUMULATIVE_SUM':
        ...
        print(f"Operation parameter (mapping set): {operation_parameter}")
        print(f"Mapping values (bucket order): {mapping_values}")
        ...
        for bucket_id in mapping_values:
            ...
            print(f"Cumulative sum for {bucket_id}: {cumulative_sum}")

        combined = cumulative_bucketed_values
    else:
        all_bucket_ids = set(dict1.keys()).union(dict2.keys())
        print(f"All bucket IDs: {all_bucket_ids}")
        for bucket_id in all_bucket_ids:
            ...
            print(f"bucket_id {bucket_id} value1 {value1} - {operation} - value2 {value2}")
            ...
            if operation == 'ADD':
                ...
                if value2 != 0:
                    ...
                    print(f"Division by zero for bucket_id {bucket_id}")
                    combined[bucket_id] = None  # Or set to zero or appropriate value
            else:
                print(f"Unknown operation: {operation}")
                combined[bucket_id] = None

    print(f"Combined bucketed values: {combined}")    
    return combined
...
print("Starting processing for each unique scenario...")
scenario_indexer = 0
for idx, scenario in grouped_scenarios.iterrows():
    ...
    print(f"Currency list: {currency_list_values}, Reporting currency: {reporting_currency}, Drill report flag: {drill_down_report_flag}, Reporting date: {reporting_date}")
    ...
    print(f"Processed currency list: {currency_list}")
    ...
    print(f'Line no 820: Merge are completed.')
    ...
    print('Line no 826: Ran get_all_dataframes_dict')  
    ...
    print("Creating mapping from rule_group to bucketing_applicability...")
            ...
            if pd.notna(rule_group):
                if rule_group in rule_group_to_bucketing_applicability:
                    if rule_group_to_bucketing_applicability[rule_group] != bucketing_applicability:
                        print(f"Warning: Inconsistent bucketing applicability for rule_group {rule_group}")
                else:
                    rule_group_to_bucketing_applicability[rule_group] = bucketing_applicability
    print("Mapping from rule_group to bucketing_applicability created.")
    ...
    print("Creating mapping from rule_group to label_ids...")
    ...
    print(f"rule_group_to_bucketing_applicability: {rule_group_to_bucketing_applicability}")
    print(f"label_id_column_to_bucketing_applicability: {label_id_column_to_bucketing_applicability}")
    ...
    def evaluate_rule_set(rule_set, rule_group):
        ...
        """
        print(f"Evaluating rule set: {rule_set}")
        ...
        if len(conditions) < 1:
            print(f"Missing rule set: {rule_set}")
        ...
        if df is None:
            print(f"Dataframe for sheet {sheet_name} is None")
            print("Returning default value 0.")
            return {}, 0
        ...
        for i in range(len(conditions)):
            ...
            print(f"before condition lenght of resultant df {len(df)}")
            condition_result = condition_result.reset_index(drop=True)
            df = df[condition_result]
            
            print(f"after condition lenght of resultant df {len(df)}")
            
            # print(f"after  {df}")
            
            if df.empty:
                print("DataFrame is empty after applying condition.")
                break
    
        if df.empty:
            print("No data after applying all conditions.")
            return {}, 0
    
        # Perform the final operation
        operation_to_perform = conditions['condition_groupby_operation'].iloc[-1].lower()
        print(f"operation_to_perform {operation_to_perform}")
        ...
        if operation_to_perform == 'weighted average' or  operation_to_perform == 'sum product':
            print("Performing weighted average or sum product  calculation")
            ...
            if weight_source_table not in all_dataframes_dict:
                print(f"Missing weight source table: {weight_source_table}")
                return {}, 0
            ...
            print(f" both table's are same {value_source_table} and {weight_source_table}")
            ...
            print(f"  ")
            print(f"value_column_full {value_column_full}")
            print(f"weight_column_full {weight_column_full}")
            print(f"  ")
            
            if value_source_table != weight_source_table :
                if merge_instructions.empty:
                    print(f"No merge instructions found for {value_source_table} and {weight_source_table}")
                    return {}, 0
                ...
                print(f"left_keys {left_keys}")
                print(f"right_keys {right_keys}")    
                ...
                print(f" name of the column's are {df.columns}")
                print(f"lenght of  df is  {len(df)}")
            ...
            if weight_column_full not in df.columns:
                print(f" weight columns not found after merge.")
                return {}, 0
            
            if value_column_full not in df.columns :
                print(f"Value columns not found after merge.")
                return {}, 0    
            ...
            try:
                ...
                print(f"Summation '{value_column_full}' took {time.time()-start:.2f}s; result={a}")
 
                start = time.time()
                b = df[weight_column_full].sum()
                print(f"Summation '{weight_column_full}' took {time.time()-start:.2f}s; result={b}")
 
                start = time.time()
                weighted_sum = (df[value_column_full] * df[weight_column_full]).sum()
                print(f"Weighted_sum took {time.time()-start:.2f}s; result={weighted_sum}")
                ...
                if operation_to_perform == 'weighted average' and total_weight != 0:
                    ...
                    print(f"weighted average final_value {final_value}")
                elif operation_to_perform == 'sum product':
                    final_value = weighted_sum
                    print(f"sum product final_value {final_value}")
                else:
                    print("Total weight is zero or unknown operation.")
                    final_value = 0
          ...
          print(f" conveting numerical values ")
        ...
        print(f"Final value for {rule_set} is {final_value}")
    
        # Apply bucketing if applicable
        global bucketed_values
        bucketed_values = {}  # Dictionary to hold bucketed values
        
        print(f"Final value for {rule_set} is {final_value}  -- bucketing_applicability {bucketing_applicability}  ")
            ...
            if not bucketing_rule_row.empty:
                ...
                if not bucketing_type_row.empty:
                    ...
                    if bucketing_type_value == 'rule_based_bucketing':
                        ...
                        print(f"Unknown bucketing type: {bucketing_type_value}")
                        bucketed_values['Unbucketed'] = final_value
                else:
                    print(f"No bucketing type found for bucketing_rule_set: {bucketing_rule_set}")
                    bucketed_values['Unbucketed'] = final_value
            else:   
                print(f"No bucketing rule mapping found for rule_id: {rule_set}")
                bucketed_values['Unbucketed'] = final_value
        else:
        ...
        print(f"Storing bucketed values in rule_def_scenario for rule_set: {rule_set}  bucketing_applicability {bucketing_applicability}")
        ...
        print(f"Returning evaluate_rule_set with bucketed_values {bucketed_values}")
        print(f"Returning evaluate_rule_set with final_value {final_value}")
    ...
    def collect_drill_down_data(df, rule_set, rule_group, value_source_column, bucketed_values):
        ...
        """
        print(f"Collecting drill-down data for rule_set: {rule_set}")
        ...
        if not currency_col_name_series.empty:
            ...
            if not unique_identifier_col_series.empty:
                ...
                print(f"No unique identifier column found for table {sheet_name}")
        else:
            print(f"No currency_column found for table {sheet_name}")
    ...
    def apply_rule_based_bucketing(df, bucketing_rule_set, value_source_column, adjustment_rule=None):
        ...
        """
        print(f"Applying rule-based bucketing for bucketing_rule_set: {bucketing_rule_set}")
            ...
            for idx, condition in bucket_conditions.iterrows():
                ...
                if 'tenor' in condition and 'tenor_unit' in condition:
                    if pd.notnull(condition['tenor']) and pd.notnull(condition['tenor_unit']):
                        ...
                        tenor = condition['tenor']
                        tenor  = int(tenor)
                        condition['tenor'] = tenor 
                        print(f" {tenor_unit} ")
                        print(f" {tenor} ")
                        print(type(tenor_unit))
                        print(type(tenor))
                        ...
                        if new_date is not None:
                            if condition['condition_datatype'] == 'Date':
                                ...
                                print(f"Calculated time to maturity: {calculated_time_to_maturity}")
                            else:
                                condition['condition_value'] = None
                                print("Failed to calculate condition value due to invalid condition dataype.")    
                        else:
                            condition['condition_value'] = None
                            print("Failed to calculate new_date due to invalid tenor_unit.")
                    else:
                        print("Tenor or tenor_unit is null.")
                else:
                    print("Tenor or tenor_unit is missing from condition.")
                
                
                print(f" apply rule based bucketing before filter lenght {len(temp_df)}")
                # print(f" print temp_df {temp_df}")
                condition_result = evaluate_condition(temp_df, condition)
                temp_df = temp_df[condition_result]
                
                print(f" apply rule based bucketing  after filter lenght {len(temp_df)}")
        ...
        if pd.notna(adjustment_rule):
            print(f"Applying adjustments using adjustment_rule: {adjustment_rule}")
            ...
            print(f"Bucketed values after adjustment: {adjusted_bucketed_values}")
            # Update df_bucketed with adjusted_df
            df_bucketed = adjusted_df
        else:
            print("No adjustment rule provided.")
        ...
        print(f"Final bucketed values: {adjusted_bucketed_values}")
    ...
    def apply_bucket_adjustments(df, value_source_column, adjustment_rule):
        ...
        """
        print(f"Applying bucket adjustments for adjustment_rule: {adjustment_rule}")
        ...
        print(f"Initial bucketed values: {initial_bucketed_values}")
    
        for idx, row in adjustments.iterrows():
            from_bucket = row['from_bucket_id']
            to_bucket = row['to_bucket_id']
            if from_bucket == to_bucket:
                print(f" Skipped as netting was taken into account the movemnet it quafiled ")
                continue
            
            impact_value = row['impact_value']
            condition_operation = row.get('condition_operation', 'And').strip().lower()
    
            print(f"Processing adjustment {idx}: from_bucket {from_bucket} to_bucket {to_bucket}")
            ...
            print(f"Condition column full name: {condition_column_full}")
    
            # Check if the column exists
            if condition_column_full not in adjusted_df.columns:
                print(f"Condition column {condition_column_full} not found in df")
                continue
            ...
            print(f"Before condition filter, from_bucket {from_bucket} has {len(df_from_bucket)} rows")
    
            if df_from_bucket.empty:
                print(f"No data in from_bucket {from_bucket}")
                continue
            ...
            print(f"After condition filter, {len(df_condition_met)} rows meet the condition")
    
            if df_condition_met.empty:
                print(f"No rows in from_bucket {from_bucket} satisfy the condition")
                continue
            ...
            print(f"Initial values in from_bucket {from_bucket} for adjustment: {initial_values.sum()}")
            print(f"Impact value: {impact_value}, amount to move: {amount_to_move.sum()}")
    
            # If from_bucket == to_bucket and impact_value == 1, no adjustment needed
            if from_bucket == to_bucket and impact_value == 1:
                print(f"No adjustment needed for from_bucket {from_bucket} to_bucket {to_bucket} with impact_value 1")
                continue
            ...
            print(f"After subtraction, total in from_bucket {from_bucket}: {adjusted_df.loc[from_bucket_mask, value_source_column].sum()}")
            ...
            if from_bucket != to_bucket:
                ...
                print(f"Moved {amount_to_move.sum()} from bucket {from_bucket} to bucket {to_bucket}")
            else:
                # When from_bucket == to_bucket and impact_value != 1
                # Adjust the values in place
                adjusted_df.loc[adjustment_indices, value_source_column] += amount_to_move
    
                print(f"Adjusted values in bucket {from_bucket} with impact_value {impact_value}")
        ...
        print(f"Final bucketed values after adjustments: {adjusted_bucketed_values}")
    ...
    def apply_static_pattern_bucketing(final_value, bucketing_rule_set):
        ...
        """
        print(f"Applying static pattern bucketing for bucketing_rule_set: {bucketing_rule_set}")
    ...
    def apply_reporting_pattern_bucketing(rule_set):
        ...
        """
        print(f"Applying reporting pattern bucketing for rule_set: {rule_set}")
        ...
        if not reporting_pattern.empty:
            ...
            if inherited_values:
                ...
                print(f"No bucketed values found to inherit from rule_id: {inherit_rule_id}")
        else:
            print(f"No reporting pattern found for rule_set: {rule_set}")
    ...
    def evaluate_rule_group(rule_group):
        ...
        """
        print(f"Evaluating rule group: {rule_group}")
        bucketing_applicability = rule_group_to_bucketing_applicability.get(rule_group, 'Yes')
        print(f"Evaluating rule group: {rule_group} and bucketing_applicability: {bucketing_applicability}")
        ...
        if len(group_def) < 1:
            print(f"Missing rule group: {rule_group}")
            print("Returning default value 0.")
        ...
        if pd.notna(group_def['final_value'].iloc[0]):
            ...
            print(f"Values are already stored")
        ...
        if len(group_def) == 1 and group_def['rule_group_operation'].iloc[0] in ['MULTIPLY']:
            ...
            if operation == 'MULTIPLY':
                ...
                if value_2 != 0 and value_2 is not None:
                    ...
                    print("Division by zero or None encountered.")
                    combined_final_value = 0
            else:
                print(f"Unknown operation: {operation}")
                combined_final_value = 0
            ...
            for _, row in group_def.iterrows():
                print(f" FOR loop start combined_final_value - {combined_final_value} at {ii}")
                ...
                if pd.notna(row['rule_set']):
                    ...
                    print("No rule_set or sub_rule_group specified.")
                    bucketed_values, value = {}, 0
    
                operation = row['rule_group_operation']
                print(f" bucketed_values : {bucketed_values} value : {value} at {ii}")
    
                # Combine bucketed values based on the operation
                print(f" operation : {operation} combined_bucketed_values : {combined_bucketed_values}")
                ...
                print(f" bucketing_applicability : {bucketing_applicability}  1069")
                print(f"Operation parameter (mapping set): {operation_parameter}  1070")
                print(f" bucketing_applicability : {bucketing_applicability}  line number  1071 ")
        
    
                if operation == 'RANGE_SUM':
                    print(f"Inside RANGE_SUM")
                    ...
                    print(f"Operation parameter (mapping set): {mapping_set_id}")
                    print(f"Mapping values (bucket IDs to sum): {mapping_values}")
                    ...
                    print(f"Inside RANGE_SUM range_sum_value {range_sum_value}")
                else:
                    combined_bucketed_values = combine_bucketed_values(
                    ...
                    if operation == 'ADD':
                        ...
                        if bucketing_applicability == 'Yes':
                            ...
                            print("Bucketing is not applicable; cannot perform cumulative sum.")
                    else:
                        print(f"Unknown operation: {operation}")
    
                print(f" FOR loop end combined_final_value - {combined_final_value} at {ii}")
                ii += 1
        ...
        print(f"Performing limit checks for rule_group: {rule_group}")
        ...
        if not limit_rows.empty:
            for idx, limit_row in limit_rows.iterrows():
                ...
                # Perform limit check with detailed print statements
                print(f"Checking limit condition for rule_group: {rule_group}")
                print(f"Value to check: {value_to_check}")
                print(f"Limit condition: {limit_condition} {limit_value}")
                ...
                if limit_condition == 'Greater than':
                    ...
                    print(f"Unknown limit_condition: {limit_condition}")
                    print(f"Breach result for rule_group {rule_group}: {breach_result}")
            ...
            print(f"No limit checks defined for rule_group: {rule_group}")
        ...
        print(f"Applying threshold criteria: {threshold_criteria} with threshold value: {threshold_value}")
        print(f"Applying threshold criteria: {threshold_criteria} with threshold value: {threshold_value}")
        try:
            threshold_value = float(threshold_value)
            print(f"Converted value to float: {threshold_value}")
        except (ValueError, TypeError):
            print(f"Passing as threshold is a string and can't be converted: {threshold_value}")
            pass
        ...
        if isinstance(threshold_value, str) and not threshold_value.replace('.', '', 1).isdigit():
            ...
            if threshold_source_table and threshold_filter_column and threshold_filter_value and threshold_value:
                print(f"Fetching dynamic threshold value from table: {threshold_source_table}")
                if threshold_source_table in all_dataframes_dict:
                    ...
                    if threshold_filter_column_full in threshold_df.columns and threshold_value_column_full in threshold_df.columns:
                        ...
                        if not filtered_df.empty:
                            ...
                            print(f"Fetched threshold value: {threshold_value}")
                        else:
                            print("No matching rows found for threshold filter.")
                            threshold_value = 1
                    else:
                        print("Threshold filter column or value column not found in threshold source table.")
                        threshold_value = 0
                else:
                    print(f"Threshold source table {threshold_source_table} not found.")
                    threshold_value = 0
            else:
                ...
                if threshold_value == "factor":
                    ...
                    for col in calculated_columns:
                        ...
                        if not factor_series.empty:
                            ...
                            print(f"Using factor from report_format: {threshold_value}")
                            break
                        else:
                            print("No factor found in report_format_scenario.")
                            threshold_value = 1  # Default factor
                else:
                    print("Threshold value is not a number or a recognized keyword.")
        ...
        if threshold_criteria == 'Greater than':
            ...
            print(f" inside smaller than")
            for bucket_id in combined_bucketed_values:
                print(f" bucket_id  {bucket_id} ")
                print(f" combined Bucked aleus  {combined_bucketed_values[bucket_id]} ")
                if combined_bucketed_values[bucket_id] >= threshold_value:
                    combined_bucketed_values[bucket_id] = 0
            # Recalculate combined_final_value based on updated bucketed values
            combined_final_value = sum(combined_bucketed_values.values())
            ...
            if threshold_value != 0:
                ...
                print("Division by zero in threshold criteria.")

        # Store the final value
        rule_group_def_scenario.loc[group_def.index, 'final_value'] = combined_final_value
    
        print(f"Just after writing evaluate_rule_group, combined_final_value: {combined_final_value}")
    
        # Store bucketed values in rule_group_def_scenario
        print(f"Storing bucketed values in rule_group_def_scenario for rule_group: {rule_group}")
        group_def_indices = group_def.index
        if bucketing_applicability == 'Yes':
            for bucket_id in bucket_ids:
        ...
        print(f"evaluate_rule_group fxn result combined_bucketed_values: {combined_bucketed_values}")
        print(f"evaluate_rule_group fxn result combined_final_value: {combined_final_value}")
    ...
    for col_name in calculated_columns:
        ...
        print(f"Calculating column: {col_name}")
        bucketing_applicability = column_type[column_type['column_name'] == col_name]['bucketing_applicability'].values[0]
        ...
        for index, row in report_format_scenario.iterrows():
            ...
            print(f"   ")
            print(f"  loop_indexer   -- {loop_indexer}  execution_order {execution_order} col_name_indexer{col_name_indexer}")
            print(f"   ")
            print(f"   ")
            
            if col_name_indexer == 1 and loop_indexer > 100:
                 # break
                pass
    ...
    # print(f"Differences calculated for calculated columns.")
    ...
    if bucket_ids:
        print(f"Number of buckets in bucket_definition: {bucket_ids}")
        for bucket_id in bucket_ids:
            print(f"Iterating over bucket: {bucket_id}")
            for col_name in calculated_columns:
  ...
  # print total time taken for the scenario
    print(f"Total time taken for scenario {scenario_analysis_id}: {time.time() - start_time} seconds")
    # End of scenario loop
print("Processing completed for all unique scenarios.")

output_final_report_format['reporting_date'] = Reporting_Date
output_final_report_format['scenario_analysis_id'] = Currency_scenario_ID
output_final_report_format['reporting_currency'] = Reporting_Currency
output_final_report_format['entity'] = Legal_entity

report_name = "IRS_Report"
dir_path = '/opt/revolutio/Platform_Configs/alm_data/'
Reporting_Date = Reporting_Date.date()

Legal_Entity = Legal_entity
file_name_output_final_report_format = f"{report_name}_{Legal_Entity}_{Reporting_Date}_{Currency_scenario_ID}_output_final_report_format.csv"
final_path_output_final_report_format = f"{dir_path}{file_name_output_final_report_format}"

file_name_rule_output = f"{report_name}_{Legal_Entity}_{Reporting_Date}_{Currency_scenario_ID}_file_name_rule_output.csv"
final_path_rule_output = f"{dir_path}{file_name_rule_output}"

file_name_rule_group_output = f"{report_name}_{Legal_Entity}_{Reporting_Date}_{Currency_scenario_ID}_file_name_rule_group_output.csv"
final_path_rule_group_output = f"{dir_path}{file_name_rule_group_output}"

file_name_drill_down_report = f"{report_name}_{Legal_Entity}_{Reporting_Date}_{Currency_scenario_ID}_file_name_drill_down_report.csv"
final_path_drill_down_report = f"{dir_path}{file_name_drill_down_report}"

print(f"Writing the {file_name_output_final_report_format} to {final_path_output_final_report_format}")
output_final_report_format.to_csv(final_path_output_final_report_format, index=False)

print(f"Writing the {file_name_rule_output} to {final_path_rule_output}")
output_rule_def_scenario.to_csv(final_path_rule_output, index=False)

print(f"Writing the {file_name_rule_group_output} to {final_path_rule_group_output}")
output_rule_group_def_scenario.to_csv(final_path_rule_group_output, index=False)

print(f"Writing the {file_name_drill_down_report} to {final_path_drill_down_report}")
drill_down_report.to_csv(final_path_drill_down_report, index=False)

output_data = output_final_report_format.astype(str)
print(f"End of {report_name} Parquet")
