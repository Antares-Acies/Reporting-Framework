...
# raise Exception(f"{config_file_name}")
...
# File locations
location = "/home/azureuser/Revolutio/Antares/Reporting Framework/"
# org_path = fr"{location}AFL SLS 20.xlsx"
org_path = rf"{location}{config_file_name}"
file_path_rule_template = rf"{location}\rule_value_{{}}.xlsx"
file_path_rule_grp_template = rf"{location}\rule_grp_value_{{}}.xlsx"
file_path_ans_template = rf"{location}\result_report_format_{{}}.xlsx"


# Ensure local directory exists
local_dir = "Platform_Configs"
if not os.path.exists(local_dir):
    os.makedirs(local_dir)

# Connect to the server
ssh_client.connect(
    hostname=hostname, username=username, password=user_secret_key, port=port
)
sftp_client = ssh_client.open_sftp()

# Ensure remote directory exists
try:
    sftp_client.stat(location)
except FileNotFoundError:
    sftp_client.mkdir(location)

# Download the workbook
# sftp_client.get(org_path, f"{local_dir}/AFL SLS 20.xlsx")
sftp_client.get(org_path, f"{local_dir}/{config_file_name}")
logging.warning("Connection successful")


# org_path = f"{local_dir}/AFL SLS 20.xlsx"
org_path = f"{local_dir}/{config_file_name}"
...
print("Reading initial dataframes...")
report_format = pd.read_excel(org_path, sheet_name="report_format")
report_format = report_format.astype("object")
...
currency_pair_master = pd.read_excel(
    org_path, sheet_name="currency_pair_master"
)
currency_conversion_master = pd.read_excel(
    org_path, sheet_name="currency_conversion_master"
)
quoted_security_data = pd.read_excel(
    org_path, sheet_name="quoted_security_data"
)
currency_scenario_config = pd.read_excel(
    org_path, sheet_name="currency_scenario_config"
)
currency_conversion_exemption = pd.read_excel(
    org_path, sheet_name="currency_conversion_exemption"
)
currency_conversion_exemption["value_source_column"] = (
    currency_conversion_exemption["value_source_table"]
    + "_+_"
    + currency_conversion_exemption["value_source_column"]
)
...
# Get the list of columns that need to be calculated
calculated_columns = column_type[column_type["calculated_column"] == "Yes"][
    "column_name"
].tolist()
...
# Read bucketing-related data
print("Reading bucketing-related data...")
...
if 'Yes' in column_type['bucketing_applicability'].values:
    ...
    bucket_rule_mapping = pd.read_excel(
        org_path, sheet_name="bucketing_rule_mapping"
    )
    bucketing_type = pd.read_excel(org_path, sheet_name="bucketing_type")
    rule_based_bucketing = pd.read_excel(
        org_path, sheet_name="rule_based_bucketing"
    )
    rule_based_bucketing["condition_column_name"] = (
        rule_based_bucketing["condition_source_table"]
        + "_+_"
        + rule_based_bucketing["condition_column_name"]
    )
    static_pattern_bucketing = pd.read_excel(
        org_path, sheet_name="static_pattern_bucketing"
    )
    reporting_pattern_bucketing = pd.read_excel(
        org_path, sheet_name="reporting_pattern_bucketing"
    )
    # Read the reporting bucketing adjustment data
    reporting_bucketing_adjustment = pd.read_excel(
        org_path, sheet_name="reporting_bucketing_adjustment"
    )
...
def read_dataframes(org_path):
    ...
    """
    unique_source_sheet = (
        rule_def_scenario["condition_source_table"].unique().tolist()
        + rule_def_scenario["value_source_table"].unique().tolist()
    )
    ...
    threshold_source_tables = (
        rule_group_def_scenario["threshold_source_table"]
        .dropna()
        .unique()
        .tolist()
    )
    ...
    for sheet in unique_source_sheet:
        try:
            ...
            sheet_data.columns = [
                f"{sheet}_+_{col}" for col in sheet_data.columns
            ]
...
def dynamic_merge(merge_master, dataframes):
    ...
    """
    merged_data = {}
    grouped_merge_instructions = merge_master.groupby(
        ["value_source_table", "condition_source_table"]
    )
    logging.warning(
        f"Grouped merge instructions: {grouped_merge_instructions.groups}"
    )
    ...
    for (left_table, right_table), group in grouped_merge_instructions:
        ...
        if right_table not in dataframes or left_table not in dataframes:
            logging.warning(
                f"One or more tables needed for merging {left_table} with {right_table} are missing."
            )
            continue
        ...
        logging.warning(
            f"before Merged {left_table} now has shape: {merged_data[left_table].shape}"
        )
        logging.warning(
            f"Before Merged {right_table} now has shape: {dataframes[right_table].shape}"
        )
        ...
        logging.warning(
            f"Merged {left_table} now has shape: {merged_data[left_table].shape}"
        )
...
def conversion(
    ...
    for index, row in base.iterrows():
        ...
        identifier_column_series = currency_conversion_master[
            currency_conversion_master["table_name"] == table_name
        ]["currency_column"]
        ...
        if not exemption_rows.empty:
            ...
            for _, exemption_row in exemption_rows.iterrows():
                exemption_condition_column = exemption_row[
                    "exemption_condition_column"
                ]
                exemption_condition_criteria = exemption_row[
                    "exemption_condition_criteria"
                ]

                # Build full column name (since columns are prefixed)
                full_exemption_condition_column = (
                    f"{table_name}_+_{exemption_condition_column}"
                )

                logging.warning(
                    f" full_exemption_condition_column {full_exemption_condition_column} "
                )

                if full_exemption_condition_column not in df.columns:
                    print(
                        f"Exemption condition column {full_exemption_condition_column} not found in table {table_name}"
                    )
                    continue

                # Build exemption mask
                exemption_mask = (
                    df[full_exemption_condition_column]
                    == exemption_condition_criteria
                )
        ...
        rates = currencies_to_convert.map(
            currency_conversion_rate.set_index("from_currency")["quoted_price"]
        )
        rates.fillna(1, inplace=True)
        df.loc[currency_mask, column_name] = (
            df.loc[currency_mask, column_name] * rates
        )
...
def evaluate_condition(df, condition):
    ...
    if condition_datatype is None:
        logging.warning(
            "KeyError: 'condition_datatype' not found in condition dictionary"
        )
    ...
    if condition_column_name not in df.columns:
        logging.warning(
            f"Failed to find the target column: {condition_column_name}"
        )
    ...
    try:
        ...
        logging.warning(
            f"Error converting column to datatype {condition_datatype}: {e}"
        )
        return pd.Series([True] * len(df), index=df.index)
    ...
    logging.warning(
        f" final condition_value post conversion's and all : {condition_value}"
    )

    if condition["condition_type"] == "Contains":
        mapping_values = mapping_set[
            mapping_set["mapping_set"] == condition_value
        ]["mapping_criteria"].tolist()
        ...
        logging.warning(
            f"Mapping values for 'Does not Contain': {mapping_values}"
        )
        return ~df_column.isin(mapping_values)
        ...
        logging.warning(
            f"Unknown condition type: {condition['condition_type']}"
        )
        # Return a Series of True with the same index as df
        return pd.Series([True] * len(df), index=df.index)
...
def filter_dataframes_by_currency(
    dataframes, currency_conversion_master, currency_list
):
    """
    ...
    for table_name, df in dataframes.items():
        currency_col_series = currency_conversion_master[
            currency_conversion_master["table_name"] == table_name
        ]["currency_column"]
...
def combine_bucketed_values(dict1, dict2, operation, operation_parameter=None):
    ...
    if operation == "CUMULATIVE_SUM":
        ...
        mapping_values = mapping_set[
            mapping_set["mapping_set"] == operation_parameter
        ]["mapping_criteria"].tolist()
        logging.warning(
            f"Operation parameter (mapping set): {operation_parameter}"
        )
        ...
        for bucket_id in all_bucket_ids:
            ...
            if operation == "MULTIPLY":
                ...
                if (
                    dict1.get(bucket_id, None) is None
                    and dict2.get(bucket_id, None) is None
                ):
                    combined_value = 0
            ...
            logging.warning(
                f"bucket_id {bucket_id} value1 {value1} - {operation} - value2 {value2}"
            )
            ...
            if operation == "ADD":
                ...
                if value2 != 0:
                    ...
                    logging.warning(
                        f"Division by zero for bucket_id {bucket_id}"
                    )
                    combined[bucket_id] = (
                        None  # Or set to zero or appropriate value
                    )
            else:
...
grouped_scenarios = (
    currency_scenario_config.groupby(
        [
            "currency_scenario_id",
            "reporting_currency",
            "drill_down_report_flag",
            "configuration_date",
        ]
    )
    .agg({"currency_list": lambda x: x.tolist()})
    .reset_index()
)
...
for idx, scenario in grouped_scenarios.iterrows():
    ...
    logging.warning(
        f"Currency list: {currency_list_values}, Reporting currency: {reporting_currency}, Drill report flag: {drill_down_report_flag}, Reporting date: {reporting_date}"
    )
    ...
    file_path_rule_grp = file_path_rule_grp_template.format(
        scenario_analysis_id
    )
    file_path_ans = file_path_ans_template.format(scenario_analysis_id)
    drill_down_file_path = (
        rf"{location}\drill_down_report_{scenario_analysis_id}.xlsx"
    )
    ...
    rule_def_scenario["condition_column_name"] = (
        rule_def_scenario["condition_source_table"]
        + "_+_"
        + rule_def_scenario["condition_column_name"]
    )
    rule_def_scenario["value_source_column"] = (
        rule_def_scenario["value_source_table"]
        + "_+_"
        + rule_def_scenario["value_source_column"]
    )
    currency_conversion_master_scenario["currency_column"] = (
        currency_conversion_master_scenario["table_name"]
        + "_+_"
        + currency_conversion_master_scenario["currency_column"]
    )

    # Prepare currency conversion data for this scenario
    currency_pair = currency_pair_master[
        currency_pair_master["to_currency"] == reporting_currency
    ]
    currency_conversion_rate = currency_pair.merge(
        quoted_security_data,
        left_on="currency_pair",
    ...
    dataframes = filter_dataframes_by_currency(
        dataframes, currency_conversion_master_scenario, currency_list
    )

    # Perform currency conversion
    base = rule_def_scenario[
        ["value_source_table", "value_source_column"]
    ].drop_duplicates()
    dataframes = conversion(
        dataframes,
        currency_conversion_master_scenario,
        currency_conversion_rate,
        base,
        reporting_currency,
    )
    ...
    # Get all dataframes
    all_dataframes_dict = get_all_dataframes_dict(
        merge_master, merged_data, dataframes
    )
    ...
    logging.warning(
        "Creating mapping from rule_group to bucketing_applicability..."
    )
        ...
        for index, row in report_format_scenario.iterrows():
            ...
            if pd.notna(rule_group):
                if rule_group in rule_group_to_bucketing_applicability:
                    if (
                        rule_group_to_bucketing_applicability[rule_group]
                        != bucketing_applicability
                    ):
                        logging.warning(
                            f"Warning: Inconsistent bucketing applicability for rule_group {rule_group}"
                        )
                else:
                    rule_group_to_bucketing_applicability[rule_group] = (
                        bucketing_applicability
    ...
    logging.warning(
        "Mapping from rule_group to bucketing_applicability created."
    )
    ...
    logging.warning(
        f"rule_group_to_bucketing_applicability: {rule_group_to_bucketing_applicability}"
    )
    logging.warning(
        f"label_id_column_to_bucketing_applicability: {label_id_column_to_bucketing_applicability}"
    )
    ...
    def evaluate_rule_set(rule_set, rule_group):
        ...
        conditions = rule_def_scenario[
            rule_def_scenario["condition_rule_set"] == rule_set
        ]
        if len(conditions) < 1:
        ...
        for i in range(len(conditions)):
            ...
            logging.warning(
                f"before condition lenght of resultant df {len(df)}"
            )
        ...
        operation_to_perform = (
            conditions["condition_groupby_operation"].iloc[-1].lower()
        )
        ...
        if (
            operation_to_perform == "weighted average"
            or operation_to_perform == "sum product"
        ):
            logging.warning(
                "Performing weighted average or sum product  calculation"
            )
            ...
            if weight_source_table not in all_dataframes_dict:
                logging.warning(
                    f"Missing weight source table: {weight_source_table}"
                )
                return {}, 0
            ...
            merge_instructions = merge_master[
                ...
                & (
                    merge_master["condition_source_table"]
                    == weight_source_table
                )
            ]

            logging.warning(
                f" both table's are same {value_source_table} and {weight_source_table}"
            )

            value_column_full = value_source_column
            weight_column_full = (
                weight_source_table + "_+_" + weight_source_column
            )
            ...
            if value_source_table != weight_source_table:
                if merge_instructions.empty:
                    logging.warning(
                        f"No merge instructions found for {value_source_table} and {weight_source_table}"
                    )
                    return {}, 0

                left_keys = [
                    value_source_table + "_+_" + key
                    for key in merge_instructions["left_key"]
                ]
                right_keys = [
                    weight_source_table + "_+_" + key
                    for key in merge_instructions["right_key"]
                ]
                ...
                if (
                    value_column_full not in df.columns
                    and weight_column_full not in df.columns
                ):  # as my dataframes are already merege not imp
            ...
            try:
                ...
                weighted_sum = (
                    df[value_column_full] * df[weight_column_full]
                ).sum()
                logging.warning(
                    f"  sum product is  num a {df[value_column_full].sum()}   "
                )
                ...
                if (
                    ...
                    logging.warning(
                        f" weighted average  final_value {final_value} "
                    )
                    ...
                    logging.warning(
                        "Total weight is zero, cannot compute weighted average."
                    )
                    final_value = 0

            except Exception as e:
                logging.warning(
                    f"Error during weighted average / sum porduct calculation: {e}"
                )
                final_value = 0

        else:
            final_value = df[conditions["value_source_column"].iloc[-1]].agg(
                operation_to_perform
            )
        ...
        logging.warning(
            f"Final value for {rule_set} is {final_value}  -- bucketing_applicability {bucketing_applicability}  "
        )
        if bucketing_applicability == "Yes":
            # Get the bucketing_rule_set for this rule_id
            bucketing_rule_row = bucket_rule_mapping[
                bucket_rule_mapping["reporting_rule_id"] == rule_set
            ]
            if not bucketing_rule_row.empty:
                ...
                bucketing_type_row = bucketing_type[
                    bucketing_type["bucketing_rule_set"] == bucketing_rule_set
                ]
                if not bucketing_type_row.empty:
                    bucketing_type_value = bucketing_type_row[
                        "bucketing_type"
                    ].values[0]
                    if bucketing_type_value == "rule_based_bucketing":
                        ...
                        bucketed_values = apply_rule_based_bucketing(
                            df,
                            bucketing_rule_set,
                            conditions["value_source_column"].iloc[-1],
                            adjustment_rule,
                        )
                    elif bucketing_type_value == "static_pattern_bucketing":
                        # Apply static pattern bucketing
                        bucketed_values = apply_static_pattern_bucketing(
                            final_value, bucketing_rule_set
                        )
                    elif bucketing_type_value == "reporting_pattern_bucketing":
                        # Apply reporting pattern bucketing
                        bucketed_values = apply_reporting_pattern_bucketing(
                            rule_set
                        )
                    else:
                        logging.warning(
                            f"Unknown bucketing type: {bucketing_type_value}"
                        )
                        bucketed_values["Unbucketed"] = final_value
                else:
                    logging.warning(
                        f"No bucketing type found for bucketing_rule_set: {bucketing_rule_set}"
                    )
                    bucketed_values["Unbucketed"] = final_value
            else:
                logging.warning(
                    f"No bucketing rule mapping found for rule_id: {rule_set}"
                )
                bucketed_values["Unbucketed"] = final_value
        else:
        ...
        logging.warning(
            f"Storing bucketed values in rule_def_scenario for rule_set: {rule_set}  bucketing_applicability {bucketing_applicability}"
        )
        ...
        logging.warning(
            f"Returning evaluate_rule_set with bucketed_values {bucketed_values}"
        )
        logging.warning(
            f"Returning evaluate_rule_set with final_value {final_value}"
        )
        return bucketed_values, final_value

    global collect_drill_down_data

    def collect_drill_down_data(
        df, rule_set, rule_group, value_source_column, bucketed_values
    ):
        """
        Collects data for the drill-down report.
        """
        logging.warning(f"Collecting drill-down data for rule_set: {rule_set}")
        sheet_name = df.columns[0].split("_+_")[
            0
        ]  # Extract table_name from column names
        ...
        if not currency_col_name_series.empty:
            ...
            if not unique_identifier_col_series.empty:
                ...
                drill_down_df["unique_identifier"] = df[
                ...
                logging.warning(
                    f"No unique identifier column found for table {sheet_name}"
                )
        else:
    ...
    def apply_bucket_adjustments(df, value_source_column, adjustment_rule):
        ...
        """
        logging.warning(
            f"Applying bucket adjustments for adjustment_rule: {adjustment_rule}"
        )
        adjustments = reporting_bucketing_adjustment[
            reporting_bucketing_adjustment["bucketing_rule_id"]
            == adjustment_rule
        ]
        adjusted_df = df.copy()

        # Log initial bucketed values
        initial_bucketed_values = (
            adjusted_df.groupby("bucket_id")[value_source_column]
            .sum()
            .to_dict()
        )
        ...
        for idx, row in adjustments.iterrows():
            ...
            if from_bucket == to_bucket:
                logging.warning(
                    f" Skipped as netting was taken into account the movemnet it quafiled "
                )
                continue

            impact_value = row["impact_value"]
            condition_operation = (
                row.get("condition_operation", "And").strip().lower()
            )

            logging.warning(
                f"Processing adjustment {idx}: from_bucket {from_bucket} to_bucket {to_bucket}"
            )
            ...
            logging.warning(
                f"Condition column full name: {condition_column_full}"
            )

            # Check if the column exists
            if condition_column_full not in adjusted_df.columns:
                logging.warning(
                    f"Condition column {condition_column_full} not found in df"
                )
                continue
            ...
            logging.warning(
                f"Before condition filter, from_bucket {from_bucket} has {len(df_from_bucket)} rows"
            )
            ...
            logging.warning(
                f"After condition filter, {len(df_condition_met)} rows meet the condition"
            )

            if df_condition_met.empty:
                logging.warning(
                    f"No rows in from_bucket {from_bucket} satisfy the condition"
                )
                continue
            ...
            initial_values = adjusted_df.loc[
                adjustment_indices, value_source_column
            ]
            amount_to_move = initial_values * impact_value

            logging.warning(
                f"Initial values in from_bucket {from_bucket} for adjustment: {initial_values.sum()}"
            )
            logging.warning(
                f"Impact value: {impact_value}, amount to move: {amount_to_move.sum()}"
            )

            # If from_bucket == to_bucket and impact_value == 1, no adjustment needed
            if from_bucket == to_bucket and impact_value == 1:
                logging.warning(
                    f"No adjustment needed for from_bucket {from_bucket} to_bucket {to_bucket} with impact_value 1"
                )
                continue
            ...
            adjusted_df.loc[adjustment_indices, value_source_column] = (
                adjusted_df.loc[adjustment_indices, value_source_column].clip(
                    lower=0
                )
            )

            logging.warning(
                f"After subtraction, total in from_bucket {from_bucket}: {adjusted_df.loc[from_bucket_mask, value_source_column].sum()}"
            )
            ...
            if from_bucket != to_bucket:
                ...
                adjusted_df = pd.concat(
                    [adjusted_df, moved_rows], ignore_index=True
                )

                logging.warning(
                    f"Moved {amount_to_move.sum()} from bucket {from_bucket} to bucket {to_bucket}"
                )
            else:
                ...
                logging.warning(
                    f"Adjusted values in bucket {from_bucket} with impact_value {impact_value}"
                )
        ...
        logging.warning(
            f"Final bucketed values after adjustments: {adjusted_bucketed_values}"
        )
    ...
    def apply_static_pattern_bucketing(final_value, bucketing_rule_set):
        ...
        """
        logging.warning(
            f"Applying static pattern bucketing for bucketing_rule_set: {bucketing_rule_set}"
        )
        static_buckets = static_pattern_bucketing[
            static_pattern_bucketing["bucketing_rule_set"] == bucketing_rule_set
        ]
        bucketed_values = {}
    ...
    def apply_reporting_pattern_bucketing(rule_set):
        ...
        """
        logging.warning(
            f"Applying reporting pattern bucketing for rule_set: {rule_set}"
        )
        reporting_pattern = reporting_pattern_bucketing[
            reporting_pattern_bucketing["rule_id"] == rule_set
        ]
        bucketed_values = {}
        ...
        if not reporting_pattern.empty:
            ...
            if inherited_values:
                ...
                logging.warning(
                    f"No bucketed values found to inherit from rule_id: {inherit_rule_id}"
                )
        else:
            logging.warning(
                f"No reporting pattern found for rule_set: {rule_set}"
            )
    ...
    def evaluate_rule_group(rule_group):
        ...
        bucketing_applicability = rule_group_to_bucketing_applicability.get(
            rule_group, "No"
        )
        logging.warning(
            f"Evaluating rule group: {rule_group} and bucketing_applicability: {bucketing_applicability}"
        )
        group_def = rule_group_def_scenario[
            rule_group_def_scenario["rule_group"] == rule_group
        ]
        if len(group_def) < 1:
        ...
        if pd.notna(group_def["final_value"].iloc[0]):
            ...
            if bucketing_applicability == "Yes":
                for bucket_id in bucket_ids:
                    value = rule_group_def_scenario.loc[
                        group_def_indices, bucket_id
                    ].values[0]
                    stored_bucketed_values[bucket_id] = (
                        value if pd.notna(value) else 0
                    )
            else:
                # If bucketing is not applicable, get the 'Unbucketed' value
                value = rule_group_def_scenario.loc[
                    group_def_indices, "Unbucketed"
                ].values[0]
                stored_bucketed_values["Unbucketed"] = (
                    value if pd.notna(value) else 0
                )
        ...
        combined_final_value = (
            None  # Start with None for operations like MULTIPLY
        )

        # For operations involving two operands, we need to handle them differently
        if len(group_def) == 1 and group_def["rule_group_operation"].iloc[
            0
        ] in ["MULTIPLY"]:
            ...
            if operation == "MULTIPLY":
                ...
                if value_2 != 0 and value_2 is not None:
                    combined_final_value = (
                        value_1 if value_1 is not None else 0
                    ) / value_2
                else:
            ...
            for _, row in group_def.iterrows():
                logging.warning(
                    f" FOR loop start combined_final_value - {combined_final_value} at {ii}"
                )

                if pd.notna(row["rule_set"]):
                    bucketed_values, value = evaluate_rule_set(
                        row["rule_set"], rule_group
                    )
                elif pd.notna(row["sub_rule_group"]):
                    # Recursively evaluate the sub-rule group and get its bucketed values and final value
                    bucketed_values, value = evaluate_rule_group(
                        row["sub_rule_group"]
                    )
                else:
                ...
                logging.warning(
                    f" bucketed_values : {bucketed_values} value : {value} at {ii}"
                )

                # Combine bucketed values based on the operation
                logging.warning(
                    f" operation : {operation} combined_bucketed_values : {combined_bucketed_values}"
                )
                ...
                logging.warning(
                    f" bucketing_applicability : {bucketing_applicability}  1069"
                )
                logging.warning(
                    f"Operation parameter (mapping set): {operation_parameter}  1070"
                )
                logging.warning(
                    f" bucketing_applicability : {bucketing_applicability}  line number  1071 "
                )
                ...
                if operation == "RANGE_SUM":
                    ...
                    mapping_values = mapping_set[
                        mapping_set["mapping_set"] == mapping_set_id
                    ]["mapping_criteria"].tolist()
                    logging.warning(
                        f"Operation parameter (mapping set): {mapping_set_id}"
                    )
                    logging.warning(
                        f"Mapping values (bucket IDs to sum): {mapping_values}"
                    )
                    # Sum the values of the specified bucket IDs from bucketed_values
                    range_sum_value = sum(
                        bucketed_values.get(bucket_id, 0)
                        for bucket_id in mapping_values
                    )
                    combined_final_value = range_sum_value
                    # For RANGE_SUM, we can set combined_bucketed_values to the summed value
                    combined_bucketed_values = {
                        "Unbucketed": combined_final_value
                    }
                    logging.warning(
                        f"Inside RANGE_SUM range_sum_value {range_sum_value}"
                    )
                else:
                    combined_bucketed_values = combine_bucketed_values(
                        combined_bucketed_values,
                    ...
                    if operation == "ADD":
                        ...
                        if value != 0 and value is not None:
                            ...
                            logging.warning(
                                "Division by zero or None encountered."
                            )
                        ...
                        if bucketing_applicability == "Yes":
                            ...
                            if bucketed_values:
                                combined_final_value = list(
                                    bucketed_values.values()
                                )[-1]
                            else:
                                combined_final_value = 0
                        else:
                            logging.warning(
                                "Bucketing is not applicable; cannot perform cumulative sum."
                            )
                    else:
                        logging.warning(f"Unknown operation: {operation}")

                logging.warning(
                    f" FOR loop end combined_final_value - {combined_final_value} at {ii}"
                )
                ii += 1
        ...
        if not limit_rows.empty:
            for idx, limit_row in limit_rows.iterrows():
                ...
                breach_column_name = (
                    column_name_breach + "_" + "breach_identifier"
                )
                ...
                logging.warning(
                    f"Checking limit condition for rule_group: {rule_group}"
                )
                logging.warning(f"Value to check: {value_to_check}")
                logging.warning(
                    f"Limit condition: {limit_condition} {limit_value}"
                )
                ...
                if limit_condition == "Greater than":
                    ...
                    logging.warning(
                        f"Unknown limit_condition: {limit_condition}"
                    )
                    logging.warning(
                        f"Breach result for rule_group {rule_group}: {breach_result}"
                    )
                # Store the breach result in rule_group_def_scenario
                rule_group_def_scenario.loc[
                    group_def.index, breach_column_name
                ] = breach_result
                # Also store in report_format_scenario
                # For all calculated columns
                for col in calculated_columns:
                    if rule_group in report_format_scenario[col].values:
                        report_format_scenario.loc[
                            report_format_scenario[col] == rule_group,
                            breach_column_name,
                        ] = breach_result
        else:
            logging.warning(
                f"No limit checks defined for rule_group: {rule_group}"
            )
        ...
        logging.warning(
            f"Applying threshold criteria: {threshold_criteria} with threshold value: {threshold_value}"
        )

        # Fetch dynamic threshold value if necessary
        if (
            isinstance(threshold_value, str)
            and not threshold_value.replace(".", "", 1).isdigit()
        ):
            # Fetch the threshold value from the specified table and filters
            if (
                threshold_source_table
                and threshold_filter_column
                and threshold_filter_value
                and threshold_value
            ):
                logging.warning(
                    f"Fetching dynamic threshold value from table: {threshold_source_table}"
                )
                if threshold_source_table in all_dataframes_dict:
                    ...
                    threshold_filter_column_full = (
                        f"{threshold_source_table}_+_{threshold_filter_column}"
                    )
                    threshold_value_column_full = (
                        f"{threshold_source_table}_+_{threshold_value}"
                    )

                    if (
                        threshold_filter_column_full in threshold_df.columns
                        and threshold_value_column_full in threshold_df.columns
                    ):
                        # Apply filter and fetch the threshold value
                        filtered_df = threshold_df[
                            threshold_df[threshold_filter_column_full]
                            == threshold_filter_value
                        ]
                        if not filtered_df.empty:
                            threshold_value = filtered_df[
                                threshold_value_column_full
                            ].iloc[0]
                            logging.warning(
                                f"Fetched threshold value: {threshold_value}"
                            )
                        else:
                            logging.warning(
                                "No matching rows found for threshold filter."
                            )
                            threshold_value = 1
                    else:
                        logging.warning(
                            "Threshold filter column or value column not found in threshold source table."
                        )
                        threshold_value = 0
                else:
                    logging.warning(
                        f"Threshold source table {threshold_source_table} not found."
                    )
                    threshold_value = 0
            else:
                ...
                if threshold_value == "factor":
                    # For all calculated columns
                    for col in calculated_columns:
                        factor_series = report_format_scenario[
                            report_format_scenario[col] == rule_group
                        ]["factor"]
                        if not factor_series.empty:
                            threshold_value = factor_series.iloc[0] / 100
                            logging.warning(
                                f"Using factor from report_format: {threshold_value}"
                            )
                            break
                        else:
                            logging.warning(
                                "No factor found in report_format_scenario."
                            )
                            threshold_value = 1  # Default factor
                else:
                    logging.warning(
                        "Threshold value is not a number or a recognized keyword."
                    )
        ...
        if threshold_criteria == "Greater than":
            ...
            for bucket_id in combined_bucketed_values:
                ...
                logging.warning(
                    f" combined Bucked aleus  {combined_bucketed_values[bucket_id]} "
                )
                if combined_bucketed_values[bucket_id] >= threshold_value:
                    combined_bucketed_values[bucket_id] = 0
            # Recalculate combined_final_value based on updated bucketed values
        ...
        rule_group_def_scenario.loc[group_def.index, "final_value"] = (
            combined_final_value
        )

        logging.warning(
            f"Just after writing evaluate_rule_group, combined_final_value: {combined_final_value}"
        )

        # Store bucketed values in rule_group_def_scenario
        logging.warning(
            f"Storing bucketed values in rule_group_def_scenario for rule_group: {rule_group}"
        )
        group_def_indices = group_def.index
        if bucketing_applicability == "Yes":
            for bucket_id in bucket_ids:
                value = combined_bucketed_values.get(bucket_id, 0)
                rule_group_def_scenario.loc[group_def_indices, bucket_id] = (
                    value
                )
        else:
            ######## If bucketing is not applicable, store 'Unbucketed' value
            rule_group_def_scenario.loc[group_def_indices, "Unbucketed"] = (
                combined_final_value
            )
        ...
        for label_id in label_ids:
            for col_name in calculated_columns:
                if (
                    report_format_scenario.loc[
                        report_format_scenario["label_id"] == label_id, col_name
                    ].values[0]
                    == rule_group
                ):
                    bucketed_values_dict[(label_id, col_name)] = (
                        combined_bucketed_values
                    )

        logging.warning(
            f"evaluate_rule_group fxn result combined_bucketed_values: {combined_bucketed_values}"
        )
        logging.warning(
            f"evaluate_rule_group fxn result combined_final_value: {combined_final_value}"
        )
    ...
    report_format_scenario.sort_values(
        "execution_order", ascending=True, inplace=True
    )
    ...
    for col_name in calculated_columns:
        ...
        bucketing_applicability = column_type[
            column_type["column_name"] == col_name
        ]["bucketing_applicability"].values[0]
        ...
        for index, row in report_format_scenario.iterrows():
            ...
            logging.warning(f"   ")
            logging.warning(
                f"  loop_indexer   -- {loop_indexer}  execution_order {execution_order} col_name_indexer{col_name_indexer}"
            )
            logging.warning(f"   ")
            logging.warning(f"   ")
    ...
    breach_columns = [
        col
        for col in report_format_scenario.columns
        if "breach_identifier" in col
    ]
    # Merge calculated values back to final_report_format
    merge_columns = (
        ["label_id"]
        + [f"final_{col}" for col in calculated_columns]
        + breach_columns
    )
    ...
    expected_columns = ["label_id"] + [
        f"expected_{col}" for col in calculated_columns
    ]
    expected_output = expected_output[expected_columns]

    final_report_format = final_report_format.merge(
        expected_output, on="label_id", how="left"
    )

    # Calculate differences between expected and final values
    for col in calculated_columns:
        final_report_format[f"{col} Diff"] = (
            final_report_format[f"final_{col}"]
            - pd.to_numeric(final_report_format[f"expected_{col}"], errors="coerce")
        )
    ...
    if bucket_ids:
        ...
        for bucket_id in bucket_ids:
            ...
            for col_name in calculated_columns:
                ...
                final_report_format[bucket_column_name] = (
                    final_report_format.apply(
                        lambda row: (
                            bucketed_values_dict.get(
                                (row["label_id"], col_name), {}
                            ).get(bucket_id, 0)
                            if label_id_column_to_bucketing_applicability.get(
                                (row["label_id"], col_name), "No"
                            )
                            == "Yes"
                            else None
                        ),
    ...
    # Save outputs
    # Ensure remote directory exists before saving
    ...
    # Log file paths
    logging.warning(f"file_path_ans: {file_path_ans}")
    logging.warning(f"file_path_rule_grp: {file_path_rule_grp}")
    logging.warning(f"file_path_rule: {file_path_rule}")

    import io
    import logging

    # Prepare memory buffers
    file_buffer_ans = io.BytesIO()
    file_buffer_rule_grp = io.BytesIO()
    file_buffer_rule = io.BytesIO()

    try:
        logging.warning("Starting to write data to memory buffers.")

        # Write data to memory buffers
        final_report_format.to_excel(file_buffer_ans, index=False)
        logging.warning(
            "Written 'final_report_format' to memory buffer successfully."
        )

        rule_group_def_scenario.to_excel(file_buffer_rule_grp, index=False)
        logging.warning(
            "Written 'rule_group_def_scenario' to memory buffer successfully."
        )

        rule_def_scenario.to_excel(file_buffer_rule, index=False)
        logging.warning(
            "Written 'rule_def_scenario' to memory buffer successfully."
        )

        # Reset buffer positions to the beginning
        file_buffer_ans.seek(0)
        logging.warning("Reset position of 'file_buffer_ans' to the beginning.")

        file_buffer_rule_grp.seek(0)
        logging.warning(
            "Reset position of 'file_buffer_rule_grp' to the beginning."
        )

        file_buffer_rule.seek(0)
        logging.warning(
            "Reset position of 'file_buffer_rule' to the beginning."
        )

        # Upload the buffers directly to SFTP
        logging.warning("Starting file upload to SFTP server.")

        sftp_client.putfo(
            file_buffer_ans,
            f"{location}result_report_format_{scenario_analysis_id}.xlsx",
        )
        logging.warning(
            f"Uploaded 'result_report_format_{scenario_analysis_id}.xlsx' to SFTP successfully."
        )

        sftp_client.putfo(
            file_buffer_rule_grp,
            f"{location}rule_grp_value_{scenario_analysis_id}.xlsx",
        )
        logging.warning(
            f"Uploaded 'rule_grp_value_{scenario_analysis_id}.xlsx' to SFTP successfully."
        )

        sftp_client.putfo(
            file_buffer_rule,
            f"{location}rule_value_{scenario_analysis_id}.xlsx",
        )
        logging.warning(
            f"Uploaded 'rule_value_{scenario_analysis_id}.xlsx' to SFTP successfully."
        )

        logging.warning("All files uploaded successfully to SFTP.")
    except Exception as e:
        logging.error(f"Error occurred during the process: {e}", exc_info=True)
    finally:
        logging.warning("Closing memory buffers.")
        # Close the buffers
        file_buffer_ans.close()
        file_buffer_rule_grp.close()
        file_buffer_rule.close()
        logging.warning("Memory buffers closed.")
    ...
    logging.warning(
        f"Total time taken for scenario {scenario_analysis_id}: {time.time() - start_time} seconds"
    )
...
sftp_client.close()
ssh_client.close()
output_data = final_report_format.astype(str)
